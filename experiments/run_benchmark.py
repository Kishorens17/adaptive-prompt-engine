...
def main(argv: list[str] | None = None) -> None:
    ...
    for i, (query, expected_type) in enumerate(BENCHMARK_QUERIES, start=1):
        ...
        records.append(_result_to_record(query, label, "adaptive_v2",   adaptive.ask(query)))
        records.append(_result_to_record(query, label, "always_medium", always_medium.ask(query)))
        records.append(_result_to_record(query, label, "no_cache",      no_cache_eng.ask(query)))
